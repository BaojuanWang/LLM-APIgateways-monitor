...
**⚠️ 764 vs 652 口径厘清(重要,防跨文档打架)**:GitHub 代码搜索采到 **764 个原始发现条目**
(含 `api.x.com` 与 `x.com` 这类同站多主机名),按 eTLD+1 归并去重后 = **652 个唯一站**;112 的差
是被折叠的重复子域。**FOFA 280 本就是唯一主域,去重不掉。** 所以:
- `764` = 发现层**原始条目数(去重前)**——briefing / 面板 / 报告 §14 / §4.1 用的是这个;
- `652` = **归并后唯一站数(去重后)**,与本表 N=1089 同口径——本说明书用这个。
- **关键:§4.1 的 one-api 占比在两种口径下逐位相同(GitHub 764→96% ≡ 652→96%;FOFA 280→71%),
  故 96/71/73 王牌论证与单位选择无关。** 论文写作时**任选其一,但全篇统一并注明口径**即可。

**【已知结果】** 来源覆盖(有重叠,不互斥,故不加总到 1089):
- 发现层 `in_disc` = 932(其中 **GitHub 发现 652(原始 764 条,去重后)· FOFA 发现 280**)
